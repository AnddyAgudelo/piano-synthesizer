...
class PianoSynthesizer:
    ...
    def create_octave_controls(self):
        octave_frame = tk.Frame(self.main_frame, bg='#444444', bd=2, relief=tk.RAISED)
        octave_frame.pack(fill=tk.X, pady=(0, 20))

        # Dividir en dos mitades
        left_frame = tk.Frame(octave_frame, bg='#444444')
        left_frame.pack(side=tk.LEFT, expand=True, fill=tk.BOTH, padx=10)

        right_frame = tk.Frame(octave_frame, bg='#444444')  # Otro color para diferenciar
        right_frame.pack(side=tk.LEFT, expand=True, fill=tk.BOTH, padx=10)

        # Controles de octava en la parte izquierda
        self.octave_label = tk.Label(left_frame, text="OCT RANGE", bg='#444444', fg="white", width=12,
                                     font=('Arial', 10))
        self.octave_label.pack(side=tk.LEFT, pady=10)

        oct_frame = tk.Frame(left_frame, bg='#444444')
        oct_frame.pack(side=tk.LEFT, padx=5)

        btn_minus = self.create_button(oct_frame, "-", command=lambda: self.change_octave(-1), width=2)
        btn_minus.pack(side=tk.LEFT, padx=5)

        self.octave_label = tk.Label(oct_frame, text=str(self.octave), bg='#444444', fg='white', width=2)
    ...
    def create_piano_keyboard(self):
        ...
        self.canvas = tk.Canvas(keyboard_frame, height=200, bg='#e0e0e0', highlightthickness=0)
    ...
    def create_control_panels(self):
        ...
        envelope_frame = self.create_panel(controls_frame, "ENVELOPES / VOLUME", "#E91E63")
        ...
        filter_frame = self.create_panel(controls_frame, "FILTER", "#CDDC39")
        ...
        # Panel de Eco (verde)
        echo_frame = self.create_panel(controls_frame, "ECHO", "#00BCD4")
    ...
    def create_panel(self, parent, title, bg_color):
        ...
        title_label = tk.Label(frame, text=title, bg='black', fg='white', font=('Arial', 12, 'bold'), width=20, pady=5)
    ...
    def create_button(self, parent, text, command=None, bg="#0077A0", fg="white", width=None, height=None):
        btn = tk.Button(parent, text=text, bg=bg, fg=fg, relief=tk.RAISED,
                        bd=2, command=command, font=('Arial', 10, 'bold'),
